...
## 20. Correções de qualidade visual e responsividade (mesma branch, PR #5, após rejeição da primeira revisão visual)

A primeira revisão manual do PR #5 identificou defeitos concretos, corrigidos nesta segunda rodada de commits sem reverter a direção de tema escuro já aprovada.

- **Correção de centralização óptica do símbolo**: a rede de nós tem limite visível real (incluindo o raio de 27px de cada nó) em y=19..292, cujo centro (155,5) ficava 9,5px acima do centro geométrico do canvas 400×330 do símbolo autônomo — e, no ícone do app, o símbolo (após `translate(112,165) scale(2.0)`) ficava 34px acima do centro do círculo de fundo, deixando uma margem inferior visivelmente maior que a superior. Corrigido ajustando apenas o enquadramento (viewBox do símbolo: `0 0 400 330` → `0 -9.5 400 330`; `translateY` do ícone do app: 165 → 199) — nenhuma coordenada de nó/trevo foi redesenhada. Documentado com os números exatos em `docs/global/LotoAtlas_BrandKit_v0.3/generate_assets.py`.
- **Novos ativos de UI transparentes**: `lotoatlas-logo-ui-reversed.svg` (lockup horizontal, sem tag de fundo, sem tagline embutida, viewBox recortado exatamente ao conteúdo real medido via `getBBox()`) e `lotoatlas-symbol-ui-on-dark.svg` (símbolo isolado, transparente, recorte simétrico) substituem `lotoatlas-logo-horizontal-reversed.svg`/`lotoatlas-symbol-on-dark.svg` no cabeçalho — eliminando o "retângulo escuro colado" que aparecia quando o fundo embutido no SVG (`#101729`) não coincidia exatamente com a superfície ao redor. Os ativos institucionais originais (com fundo, com tagline) permanecem intactos no Brand Kit para uso em apresentações/Brandbook.
- **Arquitetura de cabeçalho responsivo deliberada**: `AppShell` agora renderiza dois blocos de layout INDEPENDENTES (não uma única estrutura com `flex-wrap` genérico) — um para mobile (`sm:hidden`, três linhas: marca+navegação secundária, seletor de modalidade, indicadores de atualização) e um para desktop/tablet (`hidden sm:flex`, uma linha primária com logo+seletor+navegação, mais uma linha secundária só com os indicadores de atualização). A tagline ("Organize. Analise. Confira.") foi removida do cabeçalho de navegação em ambos os breakpoints — permanece reservada para Home/institucional/social.
- **Correção de overflow em "Detalhes técnicos" (StrategyCard)**: a grade de detalhes técnicos usava `grid-cols-2` fixo em todas as larguras. A correção para `grid-cols-1 sm:grid-cols-2` por si só não bastava: a linha "Otimiza" usava `col-span-2` incondicional, e o CSS Grid cria automaticamente uma coluna implícita quando um item pede `span 2` num grid com apenas 1 coluna explícita — reintroduzindo silenciosamente 2 colunas em qualquer largura. A correção final trocou para `sm:col-span-2` (só span 2 a partir do breakpoint em que a grade já tem 2 colunas explícitas), além de `[overflow-wrap:anywhere]` para identificadores mono longos (ex.: `megasena.max_diversification`).
- **Correção de contenção de viewport do popover "Como funciona?"**: `InfoHelp` centralizava o popover sob o gatilho (`left-1/2 -translate-x-1/2`) incondicionalmente, o que o fazia vazar da viewport quando o gatilho ficava perto de uma borda. Agora mede a posição do gatilho ao abrir (`getBoundingClientRect`) e escolhe entre alinhamento à esquerda, à direita ou centralizado, sempre com `max-w-[calc(100vw-2rem)]` como limite adicional — sem introduzir uma biblioteca de popover.
- **Open Graph / preview social**: adicionado `docs/global/LotoAtlas_BrandKit_v0.3/preview/lotoatlas-og-image.svg`/`.png` (1200×630, símbolo corrigido, "LotoAtlas", tagline, margens seguras) e as meta tags `og:image`/`twitter:image` correspondentes em `index.html` — o ícone quadrado do app deixa de ser o único candidato a imagem de preview em redes sociais/WhatsApp.

## 21. Isolamento de estado de geração entre modalidades (branch `fix/cross-modality-generation-state`)

**Bug de integridade de dados**, não apenas visual. Reprodução: abrir Lotofácil → navegar para Mega-Sena → selecionar uma estratégia e gerar → alterar a configuração (aviso de "resultado desatualizado" aparece) → usar "Voltar" do navegador. A página volta à URL `/lotofacil/gerar`, mas o resultado da Mega-Sena permanecia renderizado, e nada na Lotofácil o removia até uma nova geração.

**Causa raiz confirmada**: `/lotofacil/gerar` e `/megasena/gerar` são duas entradas de rota diferentes, mas cada uma renderiza `<AppShell><GerarPage modality="..." /></AppShell>` como o `element` de nível superior daquela rota — não há um `<Outlet>` compartilhado gerenciando remontagem por id de rota. Como `AppShell` e `GerarPage` são o mesmo tipo de componente na mesma posição da árvore em ambas as rotas, a reconciliação do React reaproveita a MESMA instância do componente ao trocar de rota, apenas atualizando a prop `modality` — em vez de desmontar e remontar. Isso significa que todo `useState` local de `GerarPage` (resultado, formulário, `lastGeneratedInput`, `resolvedSnapshot`, etc.) sobrevive à troca de modalidade. Adicionalmente, `useGenerationWorker` só encerrava o worker na troca de modalidade, sem limpar `result`/`stage`/`error` do React.

**Risco de integridade de dados**: como o resultado sobrevivia com sua própria modalidade correta embutida (`result.modality`), mas a página ao redor já mostrava a modalidade nova, era possível que "Salvar estes jogos" persistisse uma carteira com o preço/config da NOVA modalidade misturado com o resultado/instantâneo (snapshot) da modalidade ANTERIOR.

**Correção em três camadas** (nenhuma delas sozinha é suficiente, mas juntas garantem segurança mesmo se uma regredir):

1. `useGenerationWorker` (`src/shared/lib/useGenerationWorker.ts`): a limpeza do efeito de `[modality]` agora também limpa `stage`/`result`/`error`, não só encerra o worker.
2. `GerarPage` (`src/app/pages/GerarPage.tsx`): um efeito dedicado em `[modality]` reseta todo campo editável e gerado específico de modalidade (estratégia, quantidade/orçamento, concurso, dezenas fixas/excluídas, seed, preset, `lastGeneratedInput`, `resolvedSnapshot`, estados de UI de detalhes) — disparado apenas numa transição real de modalidade, nunca por uma edição dentro da mesma modalidade (o aviso de resultado desatualizado dentro da mesma modalidade é um comportamento intencional preexistente e continua idêntico). O efeito de carregamento do dataset também ganhou uma flag de cancelamento para que uma resolução tardia da modalidade anterior nunca sobrescreva o dataset/concurso sugerido da modalidade atual.
3. **Invariante de defesa em profundidade** — `activeResult = result && result.modality === modality ? result : null`: toda exibição, salvamento, cópia, exportação e comparação usa exclusivamente `activeResult`, nunca `result` bruto. Um resultado cuja modalidade não corresponde à atual nunca é acionável, mesmo que as camadas 1–2 falhem.

Como isolamento adicional (não a defesa principal), as rotas `/lotofacil/gerar` e `/megasena/gerar` em `src/app/router/router.tsx` ganharam `key="lotofacil"`/`key="megasena"`, forçando o React a desmontar/remontar `GerarPage` numa troca de rota — mas `GerarPage`/`useGenerationWorker` permanecem seguros mesmo que reaproveitados com uma prop `modality` diferente.

O design de duas capturas (`lastGeneratedInput` + `resolvedSnapshot`) que protege o resultado desatualizado DENTRO da mesma modalidade foi preservado sem alteração — o bug era de propriedade cross-modalidade, não do mecanismo de staleness em si.

## 22. Registro de aposta por jogo (branch `feat/per-ticket-bet-selection`)

Três conceitos distintos: **carteira gerada** (o conjunto completo produzido pela estratégia), **carteira salva** (persiste o conjunto COMPLETO — `SavedPortfolio.tickets` nunca perde um jogo) e **seleção de aposta real** (metadado separado sobre quais jogos salvos foram de fato apostados). Salvar ≠ apostar: o registro é opt-in explícito, para não contaminar dados observacionais futuros com carteiras salvas apenas para revisão/pesquisa.

Por que não "salvar parcialmente": as métricas/auditoria congeladas descrevem a carteira completa; remover J6 mantendo métricas calculadas para J1..J6 seria uma inconsistência matemática. Invariante obrigatório: `portfolio.tickets` = carteira gerada completa; `betSelection` = metadado.

**Modelo** (`src/shared/types/portfolio.ts`, validado em `src/shared/lib/schemas.ts`): `SavedPortfolio.betSelection?: { schemaVersion: 1, revisions: BetSelectionRevision[] }`; cada revisão tem `selectedTicketNumbers` (1-based: J1 = 1; inteiros, únicos, crescentes, dentro de 1..tickets.length — validado no schema, incluindo importação de backup), `recordedAt`, `resultAvailability` (`before_result_in_dataset` | `after_result_in_dataset` | `unknown`) e `datasetLatestContestAtRecording?`. A seleção atual é a ÚLTIMA revisão; uma revisão vazia significa "registro removido". O histórico é append-only (editar/remover acrescenta uma revisão; nada é sobrescrito). `resultAvailability` descreve se o resultado já estava no dataset do LotoAtlas no momento do registro — não prova o horário real da aposta. Editar depois do resultado gera uma revisão `after_result_in_dataset`, distinguindo registros prospectivos de retrospectivos.

**Compatibilidade**: `markedAsBet` permanece como flag agregada indexada (true quando há ao menos um jogo apostado). Legado sem `betSelection`: `markedAsBet=true` ⇒ todos os jogos apostados; `false` ⇒ sem declaração. Sem migração Dexie (o campo é opcional e não indexado); backups antigos continuam válidos e os novos fazem round-trip de todas as revisões.

**Código**: helpers puros em `src/shared/lib/betSelection.ts` (`getCurrentBetTicketNumbers`, `appendBetSelectionRevision`, `determineResultAvailability`, `summarizeBetSelectionResult`, `describeBetComparison`); `setBetSelection` em `portfolioStore`; `BetTicketPicker` (compartilhado pelo painel de salvar e pela edição); `PortfolioTicketList` ganhou uma API genérica `ticketBadges` indexada pelo número J original (a numeração é preservada entre páginas).

**Conferência**: `checkTicketsAgainstDraw` continua recebendo `portfolio.tickets` inteiro (`hitsPerTicket` alinhado a todos os jogos). A comparação apostados × não apostados é derivada apenas de `checkedResult` + seleção, é descritiva ("J6 não foi marcado como apostado e teve 1 acerto a mais que o melhor jogo apostado.") e nunca usa linguagem de decisão boa/ruim nem de prêmio. Rótulos de resultado reutilizam `formatHitResult`.

**Roadmap — OBS-001 (Registro de Pesquisa Observacional)**: os dados gravados já distinguem carteira gerada completa, jogos apostados, jogos deliberadamente não apostados, registro anterior vs. posterior à disponibilidade do resultado e o resultado de cada jogo gerado. O painel de análise NÃO faz parte deste PR; os registros permanecem locais (IndexedDB) e só saem do dispositivo se o usuário exportar um backup.
