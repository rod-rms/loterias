...
## 16. v1.1.2 — confiabilidade e robustez operacional (release concluída)

Release pequena e deliberadamente restrita a robustez/confiabilidade/robustez de repositório — sem funcionalidade nova de produto, sem mudança matemática, sem mudança de UX além de recuperação de falhas, sem integração visual da LotoAtlas. Construída a partir do `main` pós-v1.1.1.
...
- **Criação idempotente do rótulo `data-update-failure`**: o workflow não assume mais que o rótulo já existe no repositório — verifica via API (`getLabel`) e cria (`createLabel`) se ausente, tolerando com segurança a corrida de criação concorrente (HTTP 422).
...
Explicitamente fora de escopo (adiado, não implementado nesta release): migração React 19, React Router 7, upgrades maiores de Vite/Tailwind/Zod/TypeScript, modernização ampla de dependências, RMS v3, novas modalidades, funcionalidades de premiação/rateio, login, sincronização em nuvem, conferência automática de resultados históricos, reorganização de pastas, limpeza de branches remotos, integração visual do LotoAtlas Brand Kit, v1.2.

### Aceitação manual

Aceitação manual **APROVADA** no preview imutável do Cloudflare Pages para o commit `a0a587f` (https://042fcaf2.loterias-bkr.pages.dev), cobrindo: versão do aplicativo v1.1.2, geração normal de Lotofácil, geração normal de Mega-Sena, fluxo de concurso histórico, bloqueio de concurso futuro, abertura de carteira salva, conferência de resultado oficial, layout desktop e teste de fumaça mobile. A aparência visual permanecer com a identidade "Loterias" (tema claro) é esperada e não é um defeito desta release — o Brand Kit LotoAtlas é um trabalho futuro separado.

### Totais de validação (release final, HEAD `83a4540` — pós-merge de `origin/main`)

```text
npm run lint              → PASS
npm run typecheck         → PASS
npm run test:unit         → PASS (180/180 — 161 herdados da v1.1.1 + 19 novos: worker fatal-failure, error boundary, contrato do alerta do workflow incl. bootstrap do rótulo)
