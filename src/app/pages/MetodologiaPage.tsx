  ...
  return (
    <div className="max-w-none space-y-8">
      ...
      <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-5">
        <h2 className="text-lg font-semibold text-slate-900">Como este aplicativo funciona</h2>
      ...
      <section className="space-y-3 rounded-xl border border-slate-200 bg-white p-5" aria-labelledby="dados-atualizacoes">
        <h2 id="dados-atualizacoes" className="text-lg font-semibold text-slate-900">
          Dados e atualizações
        </h2>
        {statusError && <p className="text-sm text-amber-700">Não foi possível carregar as informações de atualização dos dados agora.</p>}
        {!statusError && !modalityStatus && <p className="text-sm text-slate-500">Carregando informações de atualização…</p>}
        {modalityStatus && (
          <dl className="grid grid-cols-1 gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
            <div>
              <dt className="text-xs text-slate-500">Fonte oficial</dt>
              <dd className="font-medium">
                Loterias CAIXA —{" "}
                <a href={SOURCE_URL[modality]} target="_blank" rel="noopener noreferrer" className="underline underline-offset-2 hover:text-slate-900">
                  {SOURCE_URL[modality]}
                </a>
              </dd>
            </div>
            <div>
              <dt className="text-xs text-slate-500">Último concurso na base</dt>
              <dd className="font-medium">{modalityStatus.latestContest}</dd>
            </div>
            <div>
              <dt className="text-xs text-slate-500">Data do último sorteio</dt>
              <dd className="font-medium">{modalityStatus.latestDrawDate}</dd>
            </div>
            <div>
              <dt className="text-xs text-slate-500">Situação da base</dt>
              <dd className="font-medium">{STATUS_LABEL[modalityStatus.status]}</dd>
            </div>
            <div>
              <dt className="text-xs text-slate-500">Última atualização com novo concurso</dt>
              <dd className="font-medium">{formatDateTime(modalityStatus.lastUpdatedAt)}</dd>
            </div>
            <div>
              <dt className="text-xs text-slate-500">Última verificação da fonte oficial</dt>
              <dd className="font-medium">{formatDateTime(modalityStatus.lastCheckedAt)}</dd>
            </div>
          </dl>
        )}
